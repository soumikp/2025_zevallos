...
*Last updated: 2026-05-23 | All costs in 2024 USD | Discount rate applied to costs and QALYs*
...
|---|---|---|---|---|---|---|
| Oral oncogenic HPV prevalence, men | `baseline_prev_male` | 0.033 | Proportion | Beta(106, 3097) | Giuliano et al., JAMA Otolaryngol 2023 (PROGRESS US); n=1,423 men, 21 US states, ages 18–60 | US-specific HR genotypes. Age-stratified values below used for calibration. |
...
| Parameter | Symbol in R | Value | Unit | PSA Distribution | Source | Notes |
|---|---|---|---|---|---|---|
| Annual HPV acquisition probability, men | `p_acquisition_male` | 0.041 | Annual probability | Beta(138, 3219) | Dube Mandishora et al., Nat Microbiol 2024 (HIM Study US subcohort, n=834); 3.46 per 1,000 person-months → 1−exp(−0.00346×12) = 0.041/yr | **Acquisition rate does NOT vary by age** (log-rank p=0.36). Justifies constant rate across 26–45 cohort. |
| Smoking RR on acquisition | `smoking_acquisition_RR` | 1.15 | Ratio | LogNormal(ln(1.15), 0.14) | Dube Mandishora et al., Nat Microbiol 2024, Table 4: current smoker aHR 1.15 (95% CI 0.87–1.51) | Not statistically significant. Smoking effect is primarily on clearance/persistence, not acquisition. |
...
| Probability of transitioning to persistent state at threshold crossing (ONE-SHOT) | `p_persistence_given_long_inf` | 0.020 | Proportion | Beta(2, 98) | **Calibrated 2026-05-23** to match VA HPV+ OPC incidence target from Saxena et al., J Med Econ 2022 (VA OPC burden, n=4,537 cases; all-OPC incidence 86–126/million/yr; ×0.70 HPV+ fraction → 6–9 per 100,000 PY). Grid search over {0.02, 0.03, 0.05, 0.08, 0.10, 0.15, 0.20} at 500k agents, status-quo scenario (cap=26), Weibull scale fixed at 20 (see Section 4). Result: p=0.020 → 6.59/100k PY ✓ (within 6–9 target); p=0.030 → 9.98/100k (above). Pre-calibration value from Pierce Campbell et al., Cancer Prev Res 2015 (HIM Study, n=23 HPV-16+ men, high-risk cohort) was 0.40; this is biologically appropriate for that cohort but reflects HPV-16 specifically in a high-risk sample. 0.020 reflects P(any oncogenic type → OPC-causing persistence) across the full type distribution in the broader VA male population. | **Semantics: ONE-SHOT roll at the moment `infection_duration` crosses the 2-year threshold, NOT a per-year hazard.** After a failed roll the agent remains "infected" but is flagged `persistence_evaluated = TRUE` so they are not re-rolled annually. PSA range 0.010–0.040 (reflecting calibration uncertainty). **Calibration note:** Weibull scale=20 (mean latency ~18y) was held fixed per reviewer feedback ("latency likely 10–30 years"); p_persistence was the sole lever to match incidence. See `code/calibrate_persistence.R` for full grid-search output. |
...
| OPC death attribution method | *(code logic)* | Competing risks / attributable fraction | — | — | Standard CISNET approach; Prentice et al., Biometrics 1978 | **Implemented 2026-05-23.** For each dying cancer-state agent, P(OPC cause) = excess\_rate / (background\_rate + excess\_rate). Where excess\_rate = `opc_mortality_annual` (active cancer) or `opc_survivor_excess_mortality` (survivor phase), and background\_rate = age-specific SSA qx × smoking RR (if applicable). Prior flat assignment (all cancer-state deaths → OPC) produced 99.6% case fatality — biologically implausible. Competing risks approach yields ~28% OPC-attributed mortality, consistent with HPV+ OPC 5-year OS of ~79%. |
...
| Vaccine cost (full 3-dose series) | `vaccine_cost` | 550 | 2024 USD | Gamma(5.5, 100) | CDC Vaccine Price List (Dec 1, 2024 archive): Gardasil 9 adult CDC contract price $182.79/dose × 3 doses = $548.38, rounded to $550 | VA pricing tracks federal supply schedule / CDC contract price. See Section 10 for full sourcing and CPI adjustment. Private-sector sensitivity: $923 ($307.61/dose × 3). |
...
| Parameter | Symbol in R | Value | Unit | PSA Distribution | Source | Notes |
|---|---|---|---|---|---|---|
| Number of simulated agents | `num_agents` | 1500000 | Count | — | VHA male veterans aged 26–45 (approximate policy-relevant population size). Web search on VA enrollment data confirms ~1.5M male veterans in the 26–45 age band. | **Updated 2026-05-23 from 100k.** 1.5M is the policy-relevant scale and suppresses QALY noise sufficiently for stable ICER estimation with full CRN (see below). Runtime ~100s for all 5 scenarios. Calibration runs used 500k agents. |
| Common Random Numbers (CRN) | *(code methodology)* | Shared base population + per-agent mortality draws | — | — | Standard variance-reduction technique for microsimulation; ensures scenario comparisons reflect only the policy difference, not Monte Carlo noise | **Implemented 2026-05-23.** (1) All five age-cap scenarios start from the same `generate_population()` draw, passed via `base_pop` argument. (2) A float32 matrix `mort_draws[agent_id, year]` (1.5M × 75 = ~450 MB) is pre-generated once and shared; background mortality decisions use the same uniform draw for agent *i* in year *y* regardless of scenario. Eliminates ~14,000 QALY of within-simulation noise that previously made QALY ordering non-monotone. Other events (infection, clearance, persistence roll) remain scenario-independent (minor noise contribution). Falls back to `runif()` when `mort_draws = NULL`, so calibration scripts are unaffected. |
